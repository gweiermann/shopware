...
if [ ! -f "$PLAN" ]; then
  emit_blocked "reproduction-plan.json not found — author the bundle (reproduction-plan.json + any fixtures/script) before verifying"; exit 1
fi
...
elif [ -f "$SNAP" ] && [ -n "${DATABASE_URL:-}" ]; then
  echo "== build-verify ($TARGET): resetting DB to clean snapshot + clearing cache =="
  if source "$(dirname "${BASH_SOURCE[0]}")/../lib/db-env.sh" \
...
if [ -f fixtures.json ]; then
  ...
  if ! PAYLOAD=fixtures.json bash .github/actions/repro-agent/bin/execute/seed.sh; then
...
if [ ! -f "$OUT" ]; then
  emit_blocked "the executor exited (code $rc) without producing a result — check reproduction-plan.json has a valid .executor"; exit 1
fi
