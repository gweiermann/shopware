...
case "$EXECUTOR" in
  http)
    ANALYSIS="$ANALYSIS" OUT="$OUT" bash "$BIN/execute/run-http.sh"
    ;;
  playwright)
    ANALYSIS="$ANALYSIS" OUT="$OUT" bash "$BIN/execute/run-playwright.sh"
    ;;
  direct)
    ANALYSIS="$ANALYSIS" OUT="$OUT" SHOP_DIR="${SHOP_DIR:-shop}" bash "$BIN/execute/run-direct.sh"
    ;;
  *)
