...
#      unused by direct), REPRO_PLAN (default reproduction-plan.json; ANALYSIS fallback),
...
case "$EXECUTOR" in
  http)
    ANALYSIS="$ANALYSIS" OUT="$OUT" bash .github/actions/repro-agent/bin/execute/run-http.sh
    ;;
  playwright)
    ANALYSIS="$ANALYSIS" OUT="$OUT" bash .github/actions/repro-agent/bin/execute/run-playwright.sh
    ;;
  direct)
    ANALYSIS="$ANALYSIS" OUT="$OUT" SHOP_DIR="${SHOP_DIR:-shop}" bash .github/actions/repro-agent/bin/execute/run-direct.sh
    ;;
  *)
