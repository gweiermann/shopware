...
#      REPRO_PLAN (default reproduction-plan.json; ANALYSIS compatibility fallback).
# Emits role|version|executor|admin_build|storefront_build to $GITHUB_OUTPUT (and stdout).
set -euo pipefail

: "${LEG_VERSION:?LEG_VERSION is required}"
BRANCH=${BRANCH:-trunk}
REPRO_PLAN=${REPRO_PLAN:-${ANALYSIS:-reproduction-plan.json}}
...
# demodata: the agent's decision, recorded in reproduction-plan.json (single source of truth), so
# the trunk leg provisions exactly like the reported leg did.
out demodata "$(jq -r '.fixtures.demodata // false' "$REPRO_PLAN" 2>/dev/null || echo false)"
