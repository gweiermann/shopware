...
#      OMIT_BUNDLE_DETAILS(true|false, default false).
set -euo pipefail
...
OMIT_BUNDLE_DETAILS=${OMIT_BUNDLE_DETAILS:-false}
...
headline () { case "$VERDICT" in
  ...
  needs_human_review)  [ -n "$FIX" ] && echo "Reported precondition not found — needs human review" || echo "Needs human review" ;;
...
summary () { local s; s=$(surface); local e; e=$(executor_label)
  case "$VERDICT" in
    ...
    needs_human_review)
      if [ -n "$FIX" ]; then
        echo "$(nhr_underlying) — **but the automated verdict is not trusted**${UNSURE:+: ${UNSURE}}. Candidate fix to inspect: ${FIX}. Confirm that this PR/commit removed the reported precondition before closing."
      else
        echo "$(nhr_underlying) — **but the automated verdict is not trusted**${UNSURE:+: ${UNSURE}}. The evidence below is informative; confirm the repro faithfully matches the report before acting."
      fi ;;
...
callout () { case "$VERDICT" in
  ...
  needs_human_review)
    [ -n "$FIX" ] \
      && echo "> 🟡 Treat as inconclusive — inspect ${FIX} as a possible fix candidate for the missing reported state." \
      || echo "> 🟡 Treat as unconfirmed — a human should verify the repro faithfully matches the report before acting." ;;
...
agent_explanation_section () {
  local confidence explanation rendered
  confidence=$(jq -r '.confidence // ""' "$AN" 2>/dev/null || echo "")
  explanation=$(jq -r '.agent_explanation // .confidence_reason // ""' "$AN" 2>/dev/null || echo "")
  [ -n "$explanation" ] && [ "$explanation" != null ] || return 0

  rendered="$(jcomment "$explanation")"
  echo
  echo "#### Agent Explanation"
  echo
  echo "$rendered"
  [ -n "$confidence" ] && [ "$confidence" != null ] && echo
  [ -n "$confidence" ] && [ "$confidence" != null ] && echo "**Confidence:** ${confidence}"
}
...
if [ "$OMIT_BUNDLE_DETAILS" = true ]; then
  has_script=0
  has_fixtures=0
fi
