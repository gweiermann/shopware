...
AGENT_EXPLANATION=$(jq -r '.agent_explanation // .confidence_reason // ""' "$AN" 2>/dev/null || echo "")
...
if awk "BEGIN{exit !($CONF < 0.7)}"; then
  unsure=true
  # ALWAYS record why: when the agent left no explanation, fall back to a generic confidence note —
  # otherwise the report says "not trusted" with no reason next to a clean leg result, which reads
  # as a contradiction.
  [ -z "$UNSURE_REASON" ] && UNSURE_REASON="${AGENT_EXPLANATION:-analysis confidence ${CONF} is below the 0.7 trust threshold, so the repro may not faithfully match the report}"
fi
...
# The fixing PR/commit is often already known (derived_from). Keep it for confirmed fixed verdicts
# and for uncertain/precondition-absent reports where the agent found a possible fixing change.
FIX=$(printf '%s' "$DERIVED" | tr '\n\r' '  ' | tr -s ' ' | cut -c1-300)
