...
# Writes NO file — the version/provisioning parameters are step outputs, injected into the workflow
# and prompt by deterministic steps; the agent authors reproduction-plan.json itself.
#
...
# Emits target_version, is_trunk, provision_version, composer_root_version, and
# legacy_conflicts_alias to $GITHUB_OUTPUT (and stdout).
set -euo pipefail
...
if [ "$IS_TRUNK" = true ]; then
  ...
  case "$VERSION" in
    6.6.*) out composer_root_version 6.6.x-dev ;;
    *) out composer_root_version .auto ;;
  esac
  if [ "$VERSION" = "6.6.10.0" ]; then
    out legacy_conflicts_alias true
  else
    out legacy_conflicts_alias false
  fi
fi
