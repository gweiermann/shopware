...
## What you author
Generate `ReproTest.php` and set `script_path: "ReproTest.php"` in `reproduction-plan.json`:
...
- **When the SYMPTOM is a thrown exception** (the buggy version throws — e.g. a DB error
  during indexing), ALSO set `assertion.symptom_pattern` in `reproduction-plan.json` — a distinctive
...
in `reproduction-plan.json` (see below), because synchronous indexers can throw outside your
try/catch.
