...
For Playwright UI repros, never set up state with raw Admin API calls from browser code
(`page.evaluate(fetch(...))`, `page.request`, or helper wrappers around them). Use `fixtures.json`
for static DAL rows whose ids are known before the browser run. If the scenario needs runtime state
created by the UI, such as uploaded file bytes or an id generated by an upload, create it through the
owning Admin UI flow and attach it through the owning UI flow too (for example upload/select/assign
in the product, CMS, media, or settings screen). Do not bridge a UI-created id back into setup with
an Admin API patch.
