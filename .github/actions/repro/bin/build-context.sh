...

{
  ...
  echo
  echo "- **File tools:** Read, Write, Edit, Glob, Grep. Glob/Grep are for TARGETED lookups —"
  echo "  find ONE exact template/field/selector (e.g. the Twig that renders the symptom), then"
  echo "  author. They are NOT for open-ended investigation of why the bug happens. NO sub-agents"
  echo "  (Task/Agent are disabled) — never delegate an exploration."
  ...
  echo "- **Plain read-only shell:** \`cat\` \`ls\` \`find\` \`jq\` \`grep\` \`rg\` \`head\` \`tail\` \`sed\` \`wc\` \`git log/show/diff/blame\`."
  ...
  echo "- Author in ONE pass, then verify ONCE. If build-verify is not \`reproduced\`, make AT"
  echo "  MOST ONE targeted fix (a field/selector/visibility) and re-verify once — then accept"
  echo "  or STOP. A failed verify is NOT a cue to investigate the codebase or spelunk shop-get."
