...
1. **Reindex after seeding.** `build-verify.sh` runs `dal:refresh:index` for you; nothing you seed
...
- reindex (automatic in `build-verify.sh`).

For a **product-slider / CMS** bug the same product graph applies; the slider slot's config then
references this product (or a product-stream that matches it) — keep the product graph above as the
base and add the CMS page on top, verifying the product is listable FIRST (Rule 1).
